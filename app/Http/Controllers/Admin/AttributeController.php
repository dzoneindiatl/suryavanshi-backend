...
class AttributeController extends Controller
{
   ...
   public function __construct(Request $request){
        $this->middleware('permission:view_attribute|create_attribute|edit_attribute|delete_attribute', ['only' => ['index','store']]);
    ...
    public function index(Request $request)
    {
        ...
        $DB = Attribute::query();
        $sortBy = $request->input('sortBy') ? $request->input('sortBy') : 'attributes.created_at';
        ...
        if ($request->all()) {
            ...
            if ((!empty($searchData['date_from'])) && (!empty($searchData['date_to']))) {
                ...
                $DB->whereBetween('attributes.created_at', [$dateS . " 00:00:00", $dateE . " 23:59:59"]);
            } elseif (!empty($searchData['date_from'])) {
                $dateS = $searchData['date_from'];
                $DB->where('attributes.created_at', '>=', [$dateS . " 00:00:00"]);
            } elseif (!empty($searchData['date_to'])) {
                $dateE = $searchData['date_to'];
                $DB->where('attributes.created_at', '<=', [$dateE . " 00:00:00"]);
            }
            foreach ($searchData as $fieldName => $fieldValue) {
                if ($fieldValue != "") {
                    if ($fieldName == "name") {
                        $DB->where("attributes.name", 'like', '%' . $fieldValue . '%');
                    }
                    if ($fieldName == "is_active") {
                        $DB->where("attributes.is_active", $fieldValue);
                    }
                }
            }
        }
                    ...
                    if (!empty($request->dataArr)) {
                        foreach ($request->dataArr as $variantValue) {
                            if (!empty($variantValue['name'])) {
                                $obj2   =  new AttributeValue;
                                $obj2->attribute_id = $lastId;
    ...
    public function edit($endepid)
    {
        ...
        if (!empty($endepid)) {
            ...
            $recordDetails   =   Attribute::find($record_id);

            $variantValuesData = AttributeValue::where('attribute_id', $record_id)->get()->toArray();
    ...
    // {
    //     $record_id     = base64_decode($endepid);
    //     $model = Attribute::find($record_id);
    //     if (empty($model)) {
    ...
    //                     'name' => 'required|unique:attributes,name,' . $record_id,
    //                 ),
    //                 array(
    ...
    //                             if (!empty($variantValue['name'])) {
    //                                     // Update existing
    //                                     $obj2 = AttributeValue::where('name', $variantValue['name'])
    //                                                         ->where('attribute_id', $lastId)
    //                                                         ->first();
    //                                     if ($obj2) {
    //                                         $obj2->name = $variantValue['name'];
    //                                         $obj2->save();
    ...
    //                     }
    //                    DB::commit();
    //                     Session()->flash('flash_notice', trans("Attribute updated successfully."));
    //                     return Redirect::route('admin-attributes.index');
    //                 } else {
    //                     DB::rollback();
    //                     Session()->flash('flash_notice', 'Something Went Wrong');
    ...
    public function update(Request $request, $endepid)
    {
        ...
        try {
            ...
            if (!empty($request->dataArr)) {
                ...
                $removedValues = AttributeValue::where('attribute_id', $lastId)
                                    ->whereNotIn('name', $incomingNames)
                                    ->get();

                foreach ($removedValues as $value) {

                    $isUsed = DB::table('product_attributes')
                    ...
                    if ($isUsed) {
                        ...
                        return Redirect::back()
                            ->withErrors([
                                'attribute' => "Cannot remove '{$value->name}' because it is already used by a product."
                            ])
                            ->withInput();
                    }
                }
                ...
                if ($removedValues->isNotEmpty()) {
                    AttributeValue::whereIn('id', $removedValues->pluck('id'))->delete();
                }
    ...
    public function destroy($endepid)
    {
        ...
        if ($record_id) {
             ...
             $productVData = ProductAttribute::where('attribute_id', $record_id)->first();
             if(isset($productVData)){
                  Session()->flash('flash_notice', trans("Attribute deletion is not allowed. It is associated with one or more products."));
             }else{
                Attribute::where('id', $record_id)->update(array('is_deleted' => 1));
