...
use App\Models\Blog;
use App\Models\ProductSectionManager;
use App\Models\User;
...
class ProductSectionManagersController extends Controller
{
    public $model = 'productsections';
    public function __construct(Request $request) {
        ...
        $this->listRouteName = 'admin-productsections.index';
    ...
    public function index(Request $request) {
        $DB = Blog::query();
        $sortBy = $request->input('sortBy') ? $request->input('sortBy') : 'productsections.created_at';
        ...
        if ($request->all()) {
            ...
            if ((!empty($searchData['date_from'])) && (!empty($searchData['date_to']))) {
                ...
                $DB->whereBetween('productsections.created_at', [$dateS . " 00:00:00", $dateE . " 23:59:59"]);
            } elseif (!empty($searchData['date_from'])) {
                $dateS = $searchData['date_from'];
                $DB->where('productsections.created_at', '>=', [$dateS . " 00:00:00"]);
            } elseif (!empty($searchData['date_to'])) {
                $dateE = $searchData['date_to'];
                $DB->where('productsections.created_at', '<=', [$dateE . " 00:00:00"]);
            }
            foreach ($searchData as $fieldName => $fieldValue) {
                if ($fieldValue != "") {
                    if ($fieldName == "name") {
                        $DB->where("productsections.title", 'like', '%' . $fieldValue . '%');
                    }
                    if ($fieldName == "is_active") {
                        $DB->where("productsections.is_active", $fieldValue);
                    }
                }
            }
        }
    ...
    public function create(Request $request) {
        ...
        if($request->isMethod('post')){
            try{
                $rules = [
                    'title' => 'required|unique:productsections,title',
