...
class DesignationsController extends Controller
{
    ...
    public function index(Request $request, $endesid = null)
    {
        ...
        $departmentDetails  =  Department::where('departments.id', $dep_id)->first();
        if (empty($dep_id)) {
    ...
    {   
        ...
        $departmentDetails  =  Department::where('id', $dep_id)->first();
                ...
                foreach ($formData['data'] as $data) {
                    ...
                    if (isset($data['module']) && !empty($data['module'])) {
                        ...
                        foreach ($data['module'] as $subModule) {
                            ...
                            $objData[$counter]['is_active']                   =  !empty($subModule['value']) ? $subModule['value'] : 0;
                            $counter++;
                        }
                        DesignationPermissionAction::create($objData);
                    }
                }
            }
            if (!$SavedResponse) {
                ...
                Session()->flash('success', Config('constant.DESIGNATION.DESIGNATION_TITLE') . " has been added successfully");
    ...
    public function update(Request $request, $endesid = null)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            return Redirect()->route('admin-'.$this->model . ".index", base64_encode($modell->department_id));
        }
    ...
    public function changeStatus($modelId = 0, $status = 0){
        if ($status == 1) {
            $statusMessage   =   trans(Config('constant.DESIGNATION.DESIGNATION_TITLE') . " has been deactivated successfully");
        } else {
            $statusMessage   =   trans(Config('constant.DESIGNATION.DESIGNATION_TITLE') . " has been activated successfully");
        }
    ...
    public  function delete($endesid = null)
    {
        ...
        if ($des_id) {
            Designation::where('id', $des_id)->update(array('is_deleted' => 1));
            Session()->flash('flash_notice', trans(Config('constant.DESIGNATION.DESIGNATION_TITLE') . " has been removed successfully"));
        }
        return back();
    }
}
