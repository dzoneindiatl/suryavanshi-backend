                        ...
                        <div class="col-xl-4">
                            <div class="card-body p-0">
                                <div class="mb-3">
                                    ...
                                    <select class="js-states form-control"  name="city_id" id="city_id" required>
    ...
    $(document).ready(function() {
        ...
        $('#country_id').change(function() {
            ...
            if (country_id && shipping_zone_id) {
                // Send AJAX request to get the states and existing states for all zones of the selected country
                $.ajax({
                    url: '/getstate/' + country_id,  // Endpoint to fetch states
                    method: 'GET',
                    data: {
                    ...
                    success: function(response) {
                       ...
                       // console.log('Existing States:', existingStates); // Log existing states

                        // Loop through the states and add them to the dropdown
                        $.each(response.states, function(index, state) {
                          //  console.log('State ID:', state.id); // Log the state ID

                            // Check if the state is in existingStates
                            var isDisabled = existingStates.includes(state.id) ? 'disabled' : ''; 

                            // Log the result of the check
                           // console.log('Is Disabled:', isDisabled, 'for state:', state.name);
        ...
        $('#shipping_zone_id').change(function() {
            ...
            if (country_id && shipping_zone_id) {
                // Trigger the same AJAX logic for states if both are selected
    ...
    $(document).ready(function() {
        ...
        $('#state_id').change(function() {
            ...
            if (state_ids && state_ids.length > 0) {
                ...
                $.ajax({
                    url: '/getcities',  // Laravel route to fetch cities
                    method: 'GET',
                    data: {
    ...
    $(document).ready(function() {
        ...
        function fetchShippingWeightData() {
            ...
            if (zone_id && shipping_method === 'flat') {
                ...
                $.ajax({
                    url: '/get-shipping-weight-amount-list-zone-wise',  // Laravel route for getting shipping weight list
                    method: 'GET',
