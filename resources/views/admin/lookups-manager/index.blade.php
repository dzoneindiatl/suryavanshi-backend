...
<div class="content  d-flex flex-column flex-column-fluid" id="kt_content">
	...
	<div class="d-flex flex-column-fluid">
		<div class=" container ">
        <form action="{{route($model.'.index',$type)}}" method="get" class="mws-form" autocomplete="off" enctype="multipart/form-data">
                @csrf
			<div class="row">
				<div class="col-12">
					<div class="card card-custom card-stretch card-shadowless">
						<div class="card-header">
							...
							<div class="card-toolbar">
								...
								<a href='{{route("$model.add",$type)}}'  class="btn btn-primary"> {{ trans("Add New ") }}{{ Str::studly($type) }}  </a>
							</div>
						</div>
						<div class="card-body">
									...
									<div>
											...
											<div class="row mt-8">
												<div class="col-lg-12">
													...
													&nbsp;&nbsp;												
													<a href="{{route($model.'.index',$type)}}" class="btn btn-secondary btn-secondary--icon">
														<span>
									...
									<table
										...
										<thead>
                                            ...
                                            <th class="{{(($sortBy == 'code' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'code' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',$type,array(	'sortBy' => 'code',
                                                    ...
                                                    <th class="{{(($sortBy == 'created_at' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'created_at' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',$type,array(	'sortBy' => 'created_at',
                                                    ...
                                                    <th class="{{(($sortBy == 'is_active' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'is_active' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',$type,array(	'sortBy' => 'is_active',
											...
											@if(!$results->isEmpty())
												@foreach($results as $result)
													<tr>
														...
														@if($result->is_active == 1)
                                                        <a title="Click To Deactivate" href='{{route($model.".status",array($result->id,1))}}' class="btn btn-icon btn-light btn-hover-danger btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Click To Deactivate">
                                                        ...
                                                        @else
                                                        <a title="Click To Activate" href='{{route($model.".status",array($result->id,0))}}' class="btn btn-icon btn-light btn-hover-success btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Click To Activate">
                                                        ...
                                                        @endif
														<a href="{{route($model.'.edit',array($type,base64_encode($result->id)))}}" class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="Edit">
															...
															<a href="{{route($model.'.delete',['enlokid'=>base64_encode($result->id)])}}" class="btn btn-icon btn-light btn-hover-danger btn-sm confirmDelete" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="Delete">
                                                            <span class="svg-icon svg-icon-md svg-icon-danger">
                                                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                                        <rect x="0" y="0" width="24" height="24" />
