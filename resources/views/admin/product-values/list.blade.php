        ...
        <div class="card custom-card">
            <div class="card-header justify-content-between">
                ...
                <div class="prism-toggle">
                    <a href="{{ route('admin-product-options-values-create') }}" class="btn btn-primary mb-3">Add
                        Product Option</a>
                </div>
            </div>
            <div class="card-body">
                <table id="datatable-basic" class="table table-bordered text-nowrap" style="width:100%">
                    ...
                    <tbody>
                        @forelse($productValues as $productValue)
                        <tr>
                            <td>{{ $productValue->productOption->name ?? "N/A" }}</td>
                            <td>{{ $productValue->name ?? "N/A" }}</td>
                            <td>
                                <div class="hstack gap-2 flex-wrap">
                                    <a href="{{ route('admin-product-options-values-edit',['token' => encrypt($productValue->id)]) }}"
                                        class="btn btn-info"><i class="ri-edit-line"></i></a>
                                    <form method="POST"
                                        action="{{ route('admin-product-options-values-delete',['token' => encrypt($productValue->id)]) }}">
                                        @csrf
