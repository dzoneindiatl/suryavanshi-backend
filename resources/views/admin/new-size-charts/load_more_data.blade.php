@if ($results->isNotEmpty())
    @forelse($results as $result)
        <tr class="list-data-row items-inner" data-total-count="{{ $totalResults }}" data-id = "{{ $result->id }}">
            ...
            <td>{{ $result->chest ?? 'N/A' }}</td>
            <td>{{ $result->waist ?? 'N/A' }}</td>
            <td>{{ $result->hip ?? 'N/A' }}</td>
            <td>{{ $result->shoulder ?? 'N/A' }}</td>
            <td>{{ $result->armhole ?? 'N/A' }}</td>
            <td>{{ $result->sleeve_length ?? 'N/A' }}</td>
            ...
            <td>
                <div class="hstack gap-2 flex-wrap">
                    @if ($result->is_active == 1)
                        <a href='{{ route('admin-new-size-charts.status', [$result->id, 0]) }}' class="btn btn-danger"
                            id="deactivate-button"><i class="ri-close-line"></i></a>
                    @else
                        <a href='{{ route('admin-new-size-charts.status', [$result->id, 1]) }}' class="btn btn-success"
                            id="activate-button"><i class="ri-check-line"></i></a>
                    @endif

                    <!-- <a href="{{ route('admin-new-size-charts.show', base64_encode($result->id)) }}" class="btn btn-info"><i
                    class="ri-eye-line"></i></a> -->

                    <a href="{{ route('admin-new-size-charts.edit', base64_encode($result->id)) }}"
                        class="btn btn-info"><i class="ri-edit-line"></i></a>

                    <form method="GET"
                        action="{{ route('admin-new-size-charts.delete', base64_encode($result->id)) }}">
                        @csrf
