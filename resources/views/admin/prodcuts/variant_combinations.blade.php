        ...
        <div class="card-header bg-light d-flex justify-content-between align-items-center">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input out-of-stock-toggle"
                    ...
                    id="variant_stock_toggle_{{ $primaryId }}"
                    {{ $variantCombinationOutOfStock ? 'checked' : '' }}>
               
                <label class="form-check-label" for="variant_stock_toggle_{{ $primaryId }}">
                    Out of Stock
                </label>
            </div>
        </div>
...
<script>
    ...
    $(document).off('click', '.delete-image').on('click', '.delete-image', function () {  alert('t1');
...
<script>
    function initVariantSpecializationEditors() {
        ...
        document.querySelectorAll('.ck_content').forEach(function (textarea) {
            if (!textarea.id) {
                textarea.id = 'ck_content_' + Math.random().toString(36).substr(2, 9);
            }
    ...
    onDocumentReady(function () {
        loadCkeditorScript(initVariantSpecializationEditors);
    });
</script>
