    ...
    <td>
        <div class="hstack gap-2 flex-wrap">
            @can('edit_child_category')
                @if($result->is_active == 1)
                <a href='{{route("admin-child-category.status",array($result->id,0))}}' class="btn btn-danger"
                    id="deactivate-button"><i class="ri-close-line"></i></a>
                @else
                <a href='{{route("admin-child-category.status",array($result->id,1))}}' class="btn btn-success"
                    id="activate-button"><i class="ri-check-line"></i></a>
                @endif
                
                <a href="{{route('admin-child-category.edit',base64_encode($result->id))}}" class="btn btn-info"><i
                class="ri-edit-line"></i></a>
            @endcan
            @can('delete_child_category')
                <form method="GET" action="{{route('admin-child-category.delete',base64_encode($result->id))}}">
                    @csrf
