...
@php
 ...
 @endphp
    ...
    <!-- Favicon -->
    <link rel="icon" href="<?php echo env('WEBSITE_URL') .'uploads/settings/'.@$favIcon->value; ?>" type="image/x-icon">
 ...
 <script>
    ...
    if (window.location.pathname === "/admin/category/manage-priority") {
        ...
        $('#priorityForm').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('admin-category.priority.update') }}",
                method: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    order: order
    ...
    // if (window.location.pathname === "/admin/category/manage-sub-priority") {
    if (window.location.pathname.startsWith("/admin/category/manage-sub-priority")) {
        $("#sortableCategories").sortable();
        $("#sortableCategories").disableSelection();
        ...
        $('#priorityForm').submit(function(e) {
            ...
            // Get the last segment (e.g. MTA1) from the current path
            let pathSegments = window.location.pathname.split('/');
            let parent_id = pathSegments[pathSegments.length - 1];

            $.ajax({
                url: "{{ route('admin-category.priority.subupdate') }}",
                method: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    order: order
                ...
                success: function(res) {
                    if (res.status === 'success') {
                        ...
                        // window.location.href = "{{ route('admin-category.index') }}";
                        window.location.href = "{{ url('admin/sub-category') }}/" + parent_id;
                    } else {
                        alert(res.message || 'Something went wrong');
                    }
                },
    ...
    // if (window.location.pathname === "/admin/category/manage-child-priority") {
    if (window.location.pathname.startsWith("/admin/category/manage-child-priority")) {
    $("#sortableCategories").sortable();
    $("#sortableCategories").disableSelection();
        ...
        $('#priorityForm').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('admin-category.priority.childupdate') }}",
                method: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    order: order
                ...
                success: function(res) {
                    if (res.status === 'success') {
                        ...
                        // window.location.href = "{{ route('admin-category.index') }}";
                        window.location.href = "{{ url('admin/child-category') }}/" + parent_id;
                    } else {
                        alert(res.message || 'Something went wrong');
                    }
                },
