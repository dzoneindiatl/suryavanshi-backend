...
<tr class="list-data-row" data-total-count="{{$totalResults}}">
    ...
    <td>
        <div class="hstack gap-2 flex-wrap">
            @if($result->is_active == 1)
            <a href='{{route("admin-frontend-menus.status",array($result->id,0))}}' class="btn btn-danger"
                id="deactivate-button"><i class="ri-close-line"></i></a>
            @else
            <a href='{{route("admin-frontend-menus.status",array($result->id,1))}}' class="btn btn-success"
            ...
            <a href="{{route('admin-frontend-menus.edit',base64_encode($result->id))}}" class="btn btn-info"><i
                    class="ri-edit-line"></i></a>

            <form method="GET" action="{{route('admin-frontend-menus.destroy',base64_encode($result->id))}}">
                @csrf
