@if ($results->isNotEmpty())
    @forelse($results as $result)
        <tr class="list-data-row items-inner" data-total-count="{{ $totalResults }}" data-id = "{{ $result->id }}">
            ...
            <td><a href='{{ route('admin-size-chart-tebulars-show', [$result->id]) }}' class="btn btn-success"
                    id="view-size-chart" title="Click on this to show chart">Show
                    Chart</i></a></td>
            ...
            <td>
                <div class="hstack gap-2 flex-wrap">
                    @if ($result->is_active == 1)
                        {{-- <a href='{{ route('admin-size-chart-tebulars.status', [$result->id, 0]) }}'
                            class="btn btn-danger" id="deactivate-button"><i class="ri-close-line"></i></a> --}}
                    @else
                        {{-- <a href='{{ route('admin-size-chart-tebulars.status', [$result->id, 1]) }}'
                            class="btn btn-success" id="activate-button"><i class="ri-check-line"></i></a> --}}
                    @endif

                    <!-- <a href="{{ route('admin-size-chart-tebulars.show', base64_encode($result->id)) }}" class="btn btn-info"><i
                    class="ri-eye-line"></i></a> -->

                    <a href="{{ route('admin-size-chart-tebulars.edit', base64_encode($result->id)) }}"
                        class="btn btn-info"><i class="ri-edit-line"></i></a>

                    {{-- <form method="GET"
                        action="{{ route('admin-size-chart-tebulars.delete', base64_encode($result->id)) }}">
                        @csrf
